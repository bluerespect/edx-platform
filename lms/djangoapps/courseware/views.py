...
from courseware.courses import get_courses, get_course, get_studio_url, get_course_with_access, sort_by_announcement, get_courses_by_org, find_courses_by_text
...
def filter_org(request):
    """
    Render "find courses" page.  The course selection work is done in courseware.courses.
    """
    input_org = request.GET['org']
    log.info("input org %s :", input_org)

    courses = get_courses_by_org(request.user, request.GET['org'])    
...
def find_courses(request):
    ...
    courses = find_courses_by_text(request.user, request.GET['category'], request.GET['input_text'])    
    
    log.info("@@@@@@@@@@ input_text %s :", request.GET['input_text'])
    log.info("@@@@@@@@@@ category %s :", request.GET['category'])
