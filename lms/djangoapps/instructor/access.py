...
def list_with_level(course, level):
    ...
    role = ROLES[level](course.id).users_with_role()
    return role

'''
added 05.21 for get author_name (=username)
role's type = queryset
'''

def list_with_level_username(course, level):

    role_queryset = ROLES[level](course.id).users_with_role()
    role_list = role_queryset.values_list('username')
    role_tuple = role_list[0]
    role = str(role_tuple[0])

    log.info("role!!! %s", role_tuple[0]) 
    log.info("type role!!! %s", type(role_tuple[0]))
